...
class DogRs01Robot(FanfanRobot):
    __doc__ = "12 joint actions → targets; CPG offset only if explicitly enabled."
    ...
    def __init__(self, *args, **kwargs):
        ...
        if getattr(self.cfg.control, "use_rs01_diagonal_cpg", False):
            self.rs01_cpg = RS01DiagonalCPG(
                ...
                device=(self.device),
                dt=(self.dt),
                radial_rate=(float(getattr(self.cfg.control, "cpg_radial_rate", 30.0))),
                coupling_gain=(
                    float(getattr(self.cfg.control, "cpg_coupling_gain", 18.0))
                ),
            )
    ...
    def reset_idx(self, env_ids):
        ...
        if self.episode_start_xy is not None:
            if len(env_ids) > 0:
                self.episode_start_xy[env_ids] = self.root_states[env_ids, :2]
            if self.rs01_cpg is not None:
                if len(env_ids) > 0:
    ...
    def _get_rear_pair_air_mask(self):
        ...
        return air[:, self.foot_slot_by_leg["RL"]] & air[:, self.foot_slot_by_leg["RR"]]

    def _get_front_pair_air_mask(self):
        air = self._get_foot_air_mask()
        return air[:, self.foot_slot_by_leg["FL"]] & air[:, self.foot_slot_by_leg["FR"]]
    ...
    def _get_overlapping_diagonal_air_mask(self):
        ...
        air = self._get_foot_air_mask()
        fl_rr_busy = (
            air[:, self.foot_slot_by_leg["FL"]] | air[:, self.foot_slot_by_leg["RR"]]
        )
        fr_rl_busy = (
            air[:, self.foot_slot_by_leg["FR"]] | air[:, self.foot_slot_by_leg["RL"]]
        )
    ...
    def _reward_rear_pair_air(self):
        return self._get_rear_pair_air_mask().float() * (
            1.0 - self._stand_command_gate()
        )

    def _reward_overlapping_diagonal_air(self):
        return self._get_overlapping_diagonal_air_mask().float() * (
            1.0 - self._stand_command_gate()
        )
    ...
    def _scheduled_diagonal_swing_state(self):
        ...
        opposite_support = torch.where(
            swing_fl_rr,
            support_fr_rl,
            torch.where(swing_fr_rl, support_fl_rr, torch.zeros_like(support_fl_rr)),
        )
    ...
    def _foot_velocity_relative_to_trunk(self):
        ...
        relative_world = self.feet_state[:, :, 7:10] - self.root_states[:, None, 7:10]
        foot_count = len(self.feet_indices)
        base_quat = self.base_quat[:, None, :].expand(-1, foot_count, -1).reshape(-1, 4)
        return quat_rotate_inverse(base_quat, relative_world.reshape(-1, 3)).reshape(
            self.num_envs, foot_count, 3
        )
    ...
    def _reward_forward_diagonal_pair_swing(self):
        ...
        fl_rr_forward = (
            torch.minimum(relative_velocity[:, fl, 0], relative_velocity[:, rr, 0])
            / target_swing_speed
        ).clip(0.0, 1.0)
        fr_rl_forward = (
            torch.minimum(relative_velocity[:, fr, 0], relative_velocity[:, rl, 0])
            / target_swing_speed
        ).clip(0.0, 1.0)
        pair_forward = torch.where(
            swing_fl_rr,
            fl_rr_forward,
            torch.where(swing_fr_rl, fr_rl_forward, torch.zeros_like(fl_rr_forward)),
        )
        lift_start = float(
            getattr(self.cfg.rewards, "diagonal_pair_lift_start_height", 0.015)
        )
        lift_target = max(
            float(getattr(self.cfg.rewards, "diagonal_pair_lift_target_height", 0.03)),
            lift_start + 0.0001,
        )
        lift = (
            (self.feet_pos[:, :, 2] - lift_start) / (lift_target - lift_start)
        ).clip(0.0, 1.0)
        ...
        pair_lift = torch.where(
            swing_fl_rr,
            fl_rr_lift,
            torch.where(swing_fr_rl, fr_rl_lift, torch.zeros_like(fl_rr_lift)),
        )
    ...
    def _reward_forward_progress_with_diagonal_swing(self):
        ...
        exact_scheduled = swing_fl_rr & actual_fl_rr | swing_fr_rl & actual_fr_rl
        command_x = self.commands[:, 0].clip(min=0.0)
        normalized_progress = (
            self.base_lin_vel[:, 0].clip(min=0.0) / command_x.clip(min=0.08)
        ).clip(0.0, 1.2)
        return (
    ...
    def _sagittal_torque_headroom(self):
        ...
        for leg in ("FL", "FR", "RL", "RR"):
            indices.extend(
                (self.leg_dof_indices[leg]["thigh"], self.leg_dof_indices[leg]["calf"])
            )
    ...
    def _body_angular_headroom(self):
        ...
        roll_scale = max(
            float(getattr(self.cfg.rewards, "body_headroom_roll_rate_rad_s", 1.5)),
            0.0001,
        )
        pitch_scale = max(
            float(getattr(self.cfg.rewards, "body_headroom_pitch_rate_rad_s", 1.2)),
            0.0001,
        )
        yaw_scale = max(
            float(getattr(self.cfg.rewards, "body_headroom_yaw_rate_rad_s", 2.2)),
            0.0001,
        )
        ...
        return (1.0 - torch.mean((normalized.clip(0.0, 1.0)), dim=1)).clip(0.0, 1.0)
        ...
        scales = acceleration.new_tensor(
            (
                max(
                    float(
                        getattr(
                            self.cfg.rewards, "body_headroom_roll_accel_rad_s2", 25.0
                        )
                    ),
                    0.0001,
                ),
                max(
                    float(
                        getattr(
                            self.cfg.rewards, "body_headroom_pitch_accel_rad_s2", 25.0
                        )
                    ),
                    0.0001,
                ),
                max(
                    float(
                        getattr(
                            self.cfg.rewards, "body_headroom_yaw_accel_rad_s2", 40.0
                        )
                    ),
                    0.0001,
                ),
    ...
    def _forward_progress_gate(self):
        command_x = self.commands[:, 0].clip(min=0.0)
        return (self.base_lin_vel[:, 0].clip(min=0.0) / command_x.clip(min=0.08)).clip(
            0.0, 1.0
        ) * (1.0 - self._stand_command_gate())
    ...
    def _desired_double_support_gate(self):
        ...
        return torch.all(desired_contact, dim=1).float() * (
            1.0 - self._stand_command_gate()
        )
    ...
    def _reward_hip_joint_excursion(self):
        ...
        """
        soft_limit = max(
            float(getattr(self.cfg.rewards, "hip_excursion_soft_limit_rad", 0.07)),
            0.0001,
        )
        width = max(
            float(getattr(self.cfg.rewards, "hip_excursion_penalty_width_rad", 0.09)),
            0.0001,
        )
        excess = ((torch.abs(self._hip_joint_delta()) - soft_limit) / width).clip(
            min=0.0, max=1.5
        )
        return torch.mean((torch.square(excess)), dim=1) * self._straight_motion_gate()
    ...
    def _reward_hip_target_excursion(self):
        ...
        soft_limit = max(
            float(getattr(self.cfg.rewards, "hip_target_soft_limit_rad", 0.08)), 0.0001
        )
        width = max(
            float(getattr(self.cfg.rewards, "hip_target_penalty_width_rad", 0.08)),
            0.0001,
        )
        excess = ((torch.abs(target_delta) - soft_limit) / width).clip(min=0.0, max=1.5)
        return torch.mean((torch.square(excess)), dim=1) * self._straight_motion_gate()
    ...
    def _reward_hip_policy_action_rate(self):
        ...
        return torch.mean((torch.square(delta)), dim=1) * self._straight_motion_gate()
    ...
    def _compact_hip_headroom(self):
        ...
        limit = max(
            float(getattr(self.cfg.rewards, "compact_hip_headroom_limit_rad", 0.16)),
            0.0001,
        )
        usage = (torch.abs(self._hip_joint_delta()) / limit).clip(0.0, 1.0)
        return (1.0 - torch.mean(usage, dim=1)).clip(0.0, 1.0)
    ...
    def _reward_hip_peak_excursion(self):
        ...
        soft_limit = max(
            float(getattr(self.cfg.rewards, "hip_peak_soft_limit_rad", 0.065)), 0.0001
        )
        width = max(
            float(getattr(self.cfg.rewards, "hip_peak_penalty_width_rad", 0.07)), 0.0001
        )
        excess = ((torch.abs(self._hip_joint_delta()) - soft_limit) / width).clip(
            min=0.0, max=2.0
        )
    ...
    def _hip_diagonal_motion_error(self):
        ...
        q_error = torch.stack((q[:, 0] + q[:, 3], q[:, 1] + q[:, 2]), dim=1) / 0.1
        dq_error = torch.stack((dq[:, 0] + dq[:, 3], dq[:, 1] + dq[:, 2]), dim=1) / 2.5
        return (
            0.7 * torch.mean((torch.square(q_error)), dim=1)
            + 0.3 * torch.mean((torch.square(dq_error)), dim=1)
        ).clip(max=4.0)

    def _reward_hip_diagonal_motion_mismatch(self):
        """Coordinate actual hip angles and rates within each diagonal."""
        return self._hip_diagonal_motion_error() * self._straight_motion_gate()

    def _reward_hip_trunk_twist_coupling(self):
        """Penalize rapid hip sweep specifically while the trunk is twisting."""
        hip_speed = (
            torch.mean((torch.abs(self.dof_vel[:, self.hip_dof_indices])), dim=1) / 2.5
        ).clip(0.0, 1.5)
        trunk_twist = (
            0.55 * torch.abs(self.base_ang_vel[:, 2]) / 1.5
            + 0.45 * torch.abs(self.base_ang_vel[:, 0]) / 1.0
        ).clip(0.0, 1.5)
        return hip_speed * trunk_twist * self._straight_motion_gate()

    def _reward_compact_symmetric_forward(self):
        """Reward useful motion only with compact, coordinated physical hips."""
        symmetry_headroom = (1.0 - self._hip_diagonal_motion_error() / 2.0).clip(
            0.0, 1.0
        )
        return (
            self._forward_progress_gate()
    ...
    def _reward_motor_thermal_overload(self):
        ...
        excess = (thermal_ratio - 0.8).clip(min=0.0, max=1.5)
        return (
            torch.mean((torch.square(excess)), dim=1)
            * (1.0 - self._stand_command_gate())
            * self._continuous_torque_penalty_blend()
    ...
    def _reward_motor_thermal_peak(self):
        ...
        return (
            torch.square((peak - 0.9).clip(min=0.0, max=1.5))
            * (1.0 - self._stand_command_gate())
            * self._continuous_torque_penalty_blend()
        )
    ...
    def _continuous_torque_penalty_blend(self):
        ...
        if getattr(self.cfg.env, "test", False):
            return 1.0
        curriculum_iterations = float(
            getattr(
                self.cfg.rewards, "continuous_torque_penalty_curriculum_iterations", 0.0
            )
        )
        if curriculum_iterations <= 0.0:
            return 1.0
        initial_blend = float(
            getattr(self.cfg.rewards, "continuous_torque_penalty_initial_blend", 0.0)
        )
        progress = min(
            max(self._get_torque_curriculum_iteration() / curriculum_iterations, 0.0),
            1.0,
        )
    ...
    def _reward_safe_torque_straight_progress(self):
        ...
        thermal_ratio = torch.sqrt(self.thermal_torque_sq_ema.clip(min=0.0))
        torque_headroom = (
            1.0 - torch.mean((((thermal_ratio - 0.55) / 0.55).clip(0.0, 1.0)), dim=1)
        ).clip(0.0, 1.0)
        lateral_headroom = (1.0 - torch.abs(lateral_velocity) / 0.22).clip(0.0, 1.0)
        heading_headroom = (1.0 - torch.abs(heading_error) / 0.65).clip(0.0, 1.0)
        return (
            self._forward_progress_gate()
            * torque_headroom
            * lateral_headroom
    ...
    def _straight_path_state(self):
        ...
        lateral_displacement = (
            -sin_heading * displacement[:, 0] + cos_heading * displacement[:, 1]
        )
        world_velocity = self.root_states[:, 7:9]
        lateral_velocity = (
            -sin_heading * world_velocity[:, 0] + cos_heading * world_velocity[:, 1]
        )
        heading_error = torch.atan2(
            torch.sin(heading - self.rpy[:, 2]), torch.cos(heading - self.rpy[:, 2])
        )
    ...
    def _reward_straight_path_lateral_displacement(self):
        ...
        deadband = max(
            float(getattr(self.cfg.rewards, "straight_path_lateral_deadband_m", 0.02)),
            0.0,
        )
        width = max(
            float(
                getattr(self.cfg.rewards, "straight_path_lateral_penalty_width_m", 0.18)
            ),
            0.0001,
        )
        excess = ((torch.abs(lateral_displacement) - deadband) / width).clip(
            min=0.0, max=2.0
        )
        return torch.square(excess) * self._straight_motion_gate()
    ...
    def _reward_straight_path_recovery_velocity(self):
        ...
        """
        (lateral_displacement, lateral_velocity, _) = self._straight_path_state()
        gain = float(getattr(self.cfg.rewards, "straight_path_recovery_gain_s", 1.5))
        max_velocity = max(
            float(
                getattr(
                    self.cfg.rewards, "straight_path_recovery_max_velocity_m_s", 0.12
                )
            ),
            0.0001,
        )
        desired_velocity = (-gain * lateral_displacement).clip(
            -max_velocity, max_velocity
        )
        tracking_error = ((lateral_velocity - desired_velocity) / max_velocity).clip(
            -2.0, 2.0
        )
        return torch.square(tracking_error) * self._straight_motion_gate()
    ...
    def _reward_straight_heading_recovery_rate(self):
        ...
        gain = float(getattr(self.cfg.rewards, "straight_heading_recovery_gain_s", 2.0))
        max_rate = max(
            float(
                getattr(
                    self.cfg.rewards, "straight_heading_recovery_max_rate_rad_s", 0.8
                )
            ),
            0.0001,
        )
        desired_world_yaw_rate = (gain * heading_error).clip(-max_rate, max_rate)
        tracking_error = (
    ...
    def _reward_straight_path_lateral_acceleration(self):
        ...
        valid = (self.episode_length_buf > 1).float()
        return torch.square(normalized) * self._straight_motion_gate() * valid

    def _reward_straight_balanced_progress(self):
        """Reward forward motion only when path and trunk stay controlled."""
        (_, lateral_velocity, heading_error) = self._straight_path_state()
        lateral_headroom = (1.0 - torch.abs(lateral_velocity) / 0.25).clip(0.0, 1.0)
        heading_headroom = (1.0 - torch.abs(heading_error) / 0.75).clip(0.0, 1.0)
        attitude_usage = torch.stack(
            (torch.abs(self.rpy[:, 0]) / 0.12, torch.abs(self.rpy[:, 1]) / 0.2), dim=1
        ).clip(0.0, 1.0)
        attitude_headroom = (1.0 - torch.mean(attitude_usage, dim=1)).clip(0.0, 1.0)
        return (
            self._forward_progress_gate()
            * lateral_headroom
            * heading_headroom
    ...
    def _update_cpg_diagonal_force_balance(self):
        ...
        binary contact gate.
        """
        vertical_force = self.contact_forces[:, self.feet_indices, 2].clip(min=0.0)
        nominal_weight = max(
            float(getattr(self.cfg.rewards, "transition_nominal_weight_n", 115.1)), 1.0
        )
        normalized_gain = float(
            getattr(self.cfg.control, "cpg_force_balance_gain_m_per_weight", 0.08)
        )
        max_correction = float(
            getattr(self.cfg.control, "cpg_force_balance_max_m", 0.01)
        )
        ...
        for first, second in (("FL", "RR"), ("FR", "RL")):
            ...
            correction = (
                normalized_gain * (second_force - first_force) / nominal_weight
        ...
        time_constant = max(
            float(getattr(self.cfg.control, "cpg_force_balance_time_constant_s", 0.12)),
            self.dt,
        )
        blend = self.dt / (time_constant + self.dt)
        self.cpg_leg_z_feedback += blend * (target - self.cpg_leg_z_feedback)
        self.cpg_leg_z_feedback.clamp_(min=(-max_correction), max=max_correction)
    ...
    def _contact_aware_gait_phase(self, proposed_phase):
        ...
        phase_increment = torch.remainder(proposed_phase - self.gait_phase, 1.0)
    ...
    def _compute_specialized_gait_offset(
        ...
        ramp_duration = max(
            float(getattr(self.cfg.control, "gait_transition_ramp_s", 0.2)), 0.0001
        )
        transition = (self.command_transition_age / ramp_duration).clip(0.0, 1.0)
        transition = transition * transition * (3.0 - 2.0 * transition)
        stride_gain = float(getattr(self.cfg.control, "cpg_stride_gain", 1.0))
        max_stride = float(getattr(self.cfg.control, "cpg_max_stride_m", 0.085))
        signed_stride = (
            self.commands[:, 0] * period * stance * stride_gain * transition
        ).clip(min=(-max_stride), max=max_stride)
        clearance = float(getattr(self.cfg.control, "cpg_swing_clearance_m", 0.035))
        clearance_speed = max(
            float(getattr(self.cfg.control, "cpg_full_clearance_speed_m_s", 0.12)),
            0.0001,
        )
        ...
        (foot_x, foot_z) = self.rs01_foot_trajectory.sample(
            ...
            nominal_x_m=0.0,
            nominal_z_m=(
                float(getattr(self.cfg.control, "cpg_nominal_foot_z_m", -0.3))
            ),
            lift_fraction=(float(getattr(self.cfg.control, "cpg_lift_fraction", 0.18))),
            lower_start_fraction=(
                float(getattr(self.cfg.control, "cpg_lower_start_fraction", 0.62))
            ),
        )
        foot_x += float(
            getattr(self.cfg.control, "cpg_nominal_foot_x_m", 0.0)
        ) * transition.unsqueeze(1)
        vertical_damping = float(
            getattr(self.cfg.control, "cpg_vertical_velocity_damping_s", 0.0)
        )
        if abs(vertical_damping) > 1e-08:
            max_vertical_correction = float(
                getattr(self.cfg.control, "cpg_vertical_velocity_damping_max_m", 0.006)
            )
            vertical_correction = (
                vertical_damping * self.base_lin_vel[:, 2] * transition
            ).clip(min=(-max_vertical_correction), max=max_vertical_correction)
            commanded_stance = phase < stance_ratio
            foot_z += commanded_stance.float() * vertical_correction.unsqueeze(1)
        nominal_foot_z = float(getattr(self.cfg.control, "cpg_nominal_foot_z_m", -0.3))
        ...
        if clearance_scales is not None:
            ...
            for leg in ("FL", "FR", "RL", "RR"):
                ...
                foot_z[:, foot_slot] = (
                    nominal_foot_z + float(clearance_scales[leg]) * lift[:, foot_slot]
                )

            foot_z = foot_z + self.cpg_leg_z_feedback
            fore_aft_bias = (
                float(getattr(self.cfg.control, "cpg_front_rear_load_bias_m", 0.0))
                * transition
            )
            for leg in ("FL", "FR"):
            ...
            diagonal_bias = (
                float(getattr(self.cfg.control, "cpg_diagonal_load_bias_m", 0.0))
                * transition
            )
            for leg in ("FL", "RR"):
            ...
            (thigh_target, calf_target) = self.rs01_foot_trajectory.inverse_kinematics(
                foot_x, foot_z
            )
