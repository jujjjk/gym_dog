"""B23500: explicit opt-in to physical sending and walking."""
...
def generate_launch_description():
    return LaunchDescription([
        ...
        Node(package='mydog_policy',executable='mydog_rs01_model23500_node',output='screen',parameters=[{
            'onnx_path':PathJoinSubstitution([FindPackageShare('mydog_policy'),'models','B23500.onnx']),
            **{k:LaunchConfiguration(k) for k in ('enable_send','stand_only','motor_base_url','imu_port','debug_csv_path')},
            'max_motor_age_ms':80.,'max_imu_age_sec':.06,'http_timeout_sec':.040,
