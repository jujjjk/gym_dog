#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validate the exact ONNX contract for the final hardware-balance model."""
...
from .hardware_balance_contract import MODEL_SHA256, validate_metadata
...
def main(args=None):
    ...
    print("hardware-balance ONNX validation PASSED")
    print(f"model={model}")
    print(f"sha256={digest}")
    print(
        f"input={session.get_inputs()[0].name} "
        f"{session.get_inputs()[0].shape}"
    )
    print(
        f"output={session.get_outputs()[0].name} "
    ...
    print(
        ...
        f"{contract['control']['backward_rear_calf_target_min']}"
    )
