...
本次只做代码、构建、CPU 推理和模拟接口测试，没有启动真机节点、标定 IMU、
修改电机参数或使能电机。下面的带电命令只能由操作者在吊架/系留和物理急停保护下执行。
测试通过不等于已验收自由行走。
...
- IMU 在解析校验和正确的 RAW/QUAT/EULER 数据帧后才更新时间戳；读取旧缓存不会刷新时间戳。
  要求三种帧都在 60 ms 内，帧接收时间跨度不超过 30 ms。
- 电机有效反馈年龄计入电机报告 age、HTTP 服务缓存和本地缓存，总计不超过 80 ms；
  12 路反馈的板序号/tick 必须持续前进，回退/重启/长期不变会停止。
...
- 三轴命令 `/mydog/model6850/cmd_vel`：x 前后、y 左右、z 偏航。
  首轮硬上限为 `|vx|<=0.20 m/s`、`|vy|<=0.15 m/s`、`|wz|<=0.30 rad/s`，越界拒绝并解除解锁。
- 每次运动先调用 `/mydog/model6850/arm`，必须 ready 且满足站稳门控。
  旧命令不能跨解锁继承。收到新命令且连续站稳 1 秒才进入 walk。
- 零命令、350 ms 命令超时、操作员解除解锁或实际运动满 4 秒，均从当前反馈柔和回站；
  回站后需要重新显式解锁。持续发布命令不能自动重新起步。
- 单次解锁总有效期 10 秒；控制间隔要求 10–40 ms，计算超过 40 ms 不发送旧目标；
  启动发送后，独立线程检测 150 ms 没有成功发送会请求停止。
  此软件看门狗不能覆盖进程被强杀、整机卡死或网络断开，物理急停始终必要。
- 姿态越界、反馈失效、掉线、过温、循环异常、退出均锁存故障并请求电机停止。
  急停/退出可能失去支撑，不是自动趴下。准备吊架承重后再结束。
...
辅助命令检查新鲜 ready 状态，显式申请单次解锁，以短时持续命令维持运动，
结束时自动发零命令并解除解锁。`--seconds 3` 包含约 1 秒起步门控，实际运动约 2 秒。
一次执行一条，观察回到 ready、确认稳定后再执行下一条；不要写成连续自动脚本。
