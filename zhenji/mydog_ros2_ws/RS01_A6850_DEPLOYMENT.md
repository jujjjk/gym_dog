...
2026-09-11 更新：新增独立的受保护全向硬件短测入口
`rs01_model6850.launch.py`，操作步骤见 [RS01_A6850_HARDWARE_TEST.md](RS01_A6850_HARDWARE_TEST.md)。
它接入真实反馈、柔和站立、显式单次解锁和电机发送保护；只做了离线模拟接口验证，
尚未执行带电运动验收，也不声称已验证硬件采样同步。原影子节点行为保持不变。
下面保留 2026-09-10 影子版本的接口与验证记录。
