#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Transactional NX deployment for the final hardware-balance policy.
...
class MydogHardwareBalanceNode(MydogPolicyParityFixedNode):
    ...
    def __init__(self):
        ...
        self.get_logger().warn(
            "[HARDWARE_BALANCE] command changes reset heading target "
            "without resetting gait phase or policy-action filter"
        )
        self.get_logger().warn(
            "[HARDWARE_BALANCE] ONNX owns velocity feedback and "
            "left/right symmetry; runtime adds neither a second feedback loop "
            "nor an action mirror"
        )
        self.get_logger().warn(
            "[HARDWARE_BALANCE] active torque limits real order="
            f"{active_real.tolist()}"
        )
        self.get_logger().warn(
            "[HARDWARE_BALANCE] exact backward guard active | "
            f"vx<-0.03 => RL/RR calf target >= "
            f"{BACKWARD_REAR_CALF_TARGET_MIN:+.3f}rad"
        )
    ...
    def _reset_heading_target_to_current_yaw(self, reason: str) -> None:
        ...
        self.get_logger().info(
            "[HARDWARE_BALANCE] heading anchor reset | "
    ...
    def _advance_contract_phase(self) -> None:
        # MydogPolicyParityFixedNode commits this only after a successful motor
        # send, so heading target and gait phase remain synchronized.
    ...
    def action_to_policy_target_abs_by_mode(
        ...
        """Match Gym/MuJoCo's target guard after adding the gait reference."""
        ...
        info["backward_rear_calf_target_min"] = float(
            BACKWARD_REAR_CALF_TARGET_MIN
        )
        return target, info
    ...
    def cmd_callback(self, msg):
        ...
        if self._is_significant_transition(previous_anchor, new_target):
            # Match Gym resampling: re-anchor desired heading, but preserve
            # gait phase, previous action and the action-filter state.
            self._reset_rear_torque_boost_window()
    ...
    def _guard_estimator_sync(self, obs: np.ndarray, info: dict):
        # This model contains strong velocity feedback. Replacing one stale
        # velocity frame with zero creates an artificial large tracking error.
        # Hold and gently decay the last synchronized scaled observation.
